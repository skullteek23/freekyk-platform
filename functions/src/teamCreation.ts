...
import { TeamBasicInfo, TeamMoreInfo, TeamStats, Tmember, TeamMembers } from '../../src/app/shared/interfaces/team.model';
...
export async function teamCreation(data: {
  ...
  try {
    ...
    //get
    const playerSnap = (await db.collection('players').doc(data.tcaptainId).get()).data();
    //get
    ...
    //update
    allPromises.push(db.collection('players').doc(data.tcaptainId).update({
      team: {
    ...
    allPromises.push(db.collection('teams').doc(data.newTeamInfo.id).set(newTeam));
    allPromises.push(db.collection('teams/' + data.newTeamInfo.id + '/additionalInfo').doc('moreInfo').set(teamMoreInfo));
    allPromises.push(db.collection('teams/' + data.newTeamInfo.id + '/additionalInfo').doc('statistics').set(newStats));
    allPromises.push(db.collection('teams/' + data.newTeamInfo.id + '/additionalInfo').doc('members').set(tMembers));
    // update
